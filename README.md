...
| Path | What it is |
| --- | --- |
| `apps/mobile/` | The app — Expo SDK 57 (React Native) + TypeScript + expo-router. Includes the welcome/lock screen, email→OTP→license gate and offline-persisted license state. |
| `apps/api/` | The license service (M3) — email verification + lifetime-code activation/revocation. A Cloudflare Worker + SQLite-backed Durable Object (free plan), the hard server-side gate. |
